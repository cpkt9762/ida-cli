...
impl_dispatch_tool!(DisassembleTool, DisasmRequest, "disassemble", "disassemble");
impl_dispatch_tool!(
...
impl_dispatch_tool!(
    DisassembleFunctionAtTool,
    DisasmFunctionAtRequest,
...
mod tests {
    ...
    #[tokio::test]
    async fn test_disassemble() {
        use super::*;
        let mock = MockWorker::new();
        let req = DisasmRequest {
            address: json!("0x0"),
            ..Default::default()
        };
        let result = DisassembleTool::invoke(&mock, req).await;
        assert!(result.is_ok(), "expected ok: {:?}", result);
    }

    #[tokio::test]
    async fn test_disassemble_function() {
        use super::*;
        let mock = MockWorker::new();
        let req = DisasmByNameRequest {
            name: "sub_0".to_string(),
            ..Default::default()
        };
        let result = DisassembleFunctionTool::invoke(&mock, req).await;
        assert!(result.is_ok(), "expected ok: {:?}", result);
    }
}
