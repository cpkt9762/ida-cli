...
impl_dispatch_tool!(
    GetXrefsToStringTool,
    XrefsToStringRequest,
    "get_xrefs_to_string",
    "get_xrefs_to_string"
);
impl_dispatch_tool!(
    GetXrefsToStructFieldTool,
    XrefsToFieldRequest,
...
mod tests {
    ...
    async fn test_get_xrefs_to() {
        ...
        let result = GetXrefsToTool::invoke(&mock, req).await;
        assert!(result.is_ok(), "expected ok: {:?}", result);
    }

    #[tokio::test]
    async fn test_get_xrefs_to_string() {
        use super::*;
        let mock = MockWorker::new();
        let req = XrefsToStringRequest {
            query: "hello".to_string(),
            ..Default::default()
        };
        let result = GetXrefsToStringTool::invoke(&mock, req).await;
        assert!(result.is_ok(), "expected ok: {:?}", result);
    }
}
